---
title: "Iris - An R package to quantify the tumor microenvironment based on multiplex IF data"
...
# Short example on how to use the Iris package
...
p <- overview.plot(dataset,outdir=plot_dir,palette=NULL,type='png')
```
...
neighbor.ray.plot(dataset,from_type='SOX10+ PDL1-',to_type='OTHER PD1-',plot_dir=plot_dir)
```
...
dataset <- extract.proximity(dataset,only_closest=T,radii=25)
p <- plot.proximities(dataset,"SOX10+ PDL1-",xlim_fix=3)


```

## Looking into regions of interest
```{r ROI, fig.width=7, fig.height=7}
